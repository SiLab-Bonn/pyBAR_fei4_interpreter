...
class TestAnalysis(unittest.TestCase):
    ...
    def test_trigger_data_format(self):
        ...
        for i in (0, 1, 2):  # 0: trigger data contains trigger number, 1: trigger data contains time stamp, 2: trigger data contains 15bit time stamp and 16bit trigger number
            ...
            if i == 0:
                trigger_number_ref = np.array([raw_data[0] & 0x7FFFFFFF] * len(hits), np.uint32)
                trigger_time_stamp_ref = np.array([0] * len(hits), np.uint32)
            elif i == 1:
                trigger_number_ref = np.array([0] * len(hits), np.uint32)
                trigger_time_stamp_ref = np.array([raw_data[0] & 0x7FFFFFFF] * len(hits), np.uint32)
            elif i == 2:
                trigger_number_ref = np.array([raw_data[0] & 0x0000FFFF] * len(hits), np.uint32)
                trigger_time_stamp_ref = np.array([(raw_data[0] & 0x7FFF0000) >> 16] * len(hits), np.uint32)
